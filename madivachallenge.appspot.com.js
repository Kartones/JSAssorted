...
(function firstChallenge() {
  var isBlack = function (node) {
    ...
    return (
      document.defaultView
        ...
        .getPropertyValue("color") == "rgb(0, 0, 0)"
    );
  };
  var number_nodes = [];
  ...
  for (i = 0, len = nodes.length; i < len; i++) {
    if (nodes[i].childNodes.length > 1) {
      number = null;
      if (
        nodes[i].childNodes[0].childNodes.length > 0 &&
        isBlack(nodes[i].childNodes[0])
      ) {
        number = parseInt(nodes[i].childNodes[0].innerHTML.trim(), 10);
      } else {
        if (
          nodes[i].childNodes[1].childNodes.length > 0 &&
          isBlack(nodes[i].childNodes[1])
        ) {
  ...
  return number_nodes.reduce(function (prev, current) {
    return prev + current;
  }, 0);
})();
